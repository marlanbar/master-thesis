...
GT = pd.read_csv(config.DATA_PROCESSED + "humsavar_gt.csv.gz", index_col="MUTANT")
...
sequences = open(config.DATA_INTERIM + "human_prot_sequences.txt", "w")
for e, record in enumerate(SeqIO.parse(config.DATA_EXTERNAL + "uniprot-proteome%3AUP000005640.fasta", "fasta")):
    uni_id = record.id.split("|")[1]
    sequences.write("%s,%s\n" % (uni_id, record.seq))
    
    
sequences = pd.read_csv(config.DATA_INTERIM + "human_prot_sequences.txt", header=None, names=["uniprot", "sequence"])
...
try:
    uniprots = pd.read_csv(config.DATA_INTERIM + "humsavar_gt_protein_slices.csv.gz")
except:    
    ...
    uniprots[["var_slice", "slice"]].to_csv(config.DATA_INTERIM + "humsavar_gt_protein_slices.csv.gz", 
...
uniprots.drop("SLICE", 1).drop("VAR_SLICE",1).to_csv(config.DATA_INTERIM + "protparam_features.csv.gz", index=False, compression="gzip")
